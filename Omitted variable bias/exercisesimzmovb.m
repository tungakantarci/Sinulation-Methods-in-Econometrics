...
%% 2. Create the sampling distribution of the biased OLS estimator 
...
% 2.5. Create the constant term
x_0 = ones(N_obs,1);

% 2.6. Create a vector of covariances between two independent variables
sigma_x_1_x_2 = 0:0.11:0.99;
N_sig = size(sigma_x_1_x_2,2);

% 2.7. Preallocate a matrix for storing OLS estimates from all samples
B_hat_1_sim = NaN(N_sim,N_sig);

% 2.8. Define mean vector for the multivariate random number generator 
mu = [0 0];

% 2.9. Coefficient estimates
for j = 1:N_sig
    for i = 1:N_sim
        ...
        X_including_x_2 = [x_0 x_1 x_2];
        y = X_including_x_2*B_true+u;
        X_excluding_x_2 = [x_0 x_1];
        LSS = exercisefunctionlss(y,X_excluding_x_2); 
        B_hat_1_sim(i,j) = LSS.B_hat(2,1); 
    end
end
